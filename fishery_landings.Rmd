...
#### Remove inconsistent low participation fisheries from trends
* bluefish, surf clam, sea cucumber, American eel,winter flounder, yellowtail flounder, hagfish, silver hake (whiting), Atlantic periwinkles (cockles), green sea urchin, seaweed, skates, smelts, long-finned squid, short-finned squid, swordfish, bigeye tuna,Atlantic wolffish

* Should we remove low participation, but consistent fisheries? Example: bloodworms, razor clam, cusk, spiny dogfish, elvers, halibut, European flat oyster, sandworms, bluefin tuna, waved whelk
```{r}
#remove species blanks, other, unclassified
species_filter1<-filter(species, species != "")%>%
...
species_filter2<-species_filter1%>%
  filter(species != "Bluefish")%>%
  filter(species != "Clam Surf / Hen")%>%
  filter(species != "Cucmber Sea")%>%
  filter(species != "Eel American")%>%
  filter(species != "Flounder Winter")%>%
  filter(species != "Flounder Yellowtail")%>%
  filter(species != "Hagfish")%>%
  filter(species != "Hake Silver (Whiting)")%>%
  filter(species !="Periwinkles Atlantic (Cockles)")%>%
  filter(species != "Sea Urchins Green")%>%
  filter(species !="Skates")%>%
  filter(species !="Smelts")%>%
  filter(species != "Squid Long Finned")%>%
  filter(species != "Squid Short Finned")%>%
  filter(species != "Swordfish")%>%
  filter(species != "Tuna Bigeye")%>%
  filter(species != "Wolffish Atlantic")

for(i in 1:5){

print(ggplot()+
  geom_point(data=species_filter2, aes(x=year, y=1, size=total_weight))+
  theme(axis.text.y = element_blank(), axis.title.y = element_blank(), axis.ticks.y=element_blank())+
  scale_size(range = c(.1,12), name="Weight")+
  facet_wrap_paginate(~species, ncol=2, nrow=4, page=i))
}
...
#### Remove inconsistent low participation fisheries from trends
* Sea cucumber, winter flounder, yellowtail flounder,silver hake, skates, wolffish, crab unclassified

```{r}
#remove species blanks, other, unclassified
species_filter1<-filter(species_full, species != "Cucumber Sea")%>%
  filter(species != "Flounder Winter")%>%
  filter(species != "Flounder Yellowtail")%>%
  filter(species != "Hake Silver (Whiting)")%>%
  filter(species != "Skates")%>%
...
table(landings_missing$COMMON_NAME)
